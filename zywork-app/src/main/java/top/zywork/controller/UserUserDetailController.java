...
public class UserUserDetailController extends BaseController {
    ...
    @PostMapping("user/weixin-friend")
    public ResponseStatusVO weixinFriend() {
        JwtUser jwtUser = SecurityUtils.getJwtUser();
        if (jwtUser == null) {
            return ResponseStatusVO.authenticationError();
        }

        UserUserDetailQuery userUserDetailQuery = new UserUserDetailQuery();
        userUserDetailQuery.setUserId(jwtUser.getUserId());
        PagerDTO pagerDTO = userUserDetailService.weiXinFriend(userUserDetailQuery);
