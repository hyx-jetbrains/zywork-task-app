...
<script>
	...
	import {listJoin} from '../../common/weixin-task.js'
	export default {
		data() {
			return {
				weixinTaskList: [],
		...
		onPullDownRefresh() {
			this.pager.pageNo = 1
			listJoin(this, 'pullDown')
		},
		onReachBottom() {
			this.showLoadMore = true
			this.pager.pageNo = this.pager.pageNo + 1
			listJoin(this, 'reachBottom')
		},
		methods: {
			toWeixinTaskDetail(item) {
				uni.navigateTo({
					url: '/pages/weixin-task-detail/weixin-task-detail?id=' + item.weixinTaskId + '&taskFrom=join'
				})
			}
		}
	}
